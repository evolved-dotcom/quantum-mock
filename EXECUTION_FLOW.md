...
**Paso 19 — Debounce**: `loadRules()` programa un `setTimeout` de 150ms. Si llegan más señales `RULES_UPDATED` durante জ্ঞesos 150ms (porque se están grabando peticiones en ráfaga), cada nueva señal reinicia el temporizador. Solo cuando hay 150ms de silencio, se ejecuta `executeLoad()`.
